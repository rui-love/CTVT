...
def generate_spiral2d(
    ...
    for _ in range(nspiral):
        ...
        t0_idx = npr.multinomial(
            1, [1.0 / (ntotal - 2.0 * nsample)] * (ntotal - int(2 * nsample))
        )
        t0_idx = np.argmax(t0_idx) + nsample

        cc = bool(npr.rand() > 0.5)  # uniformly select rotation
...
if __name__ == "__main__":
    ...
    if args.visualize:
        with torch.no_grad():
            ...
            ts_pos = np.linspace(0.0, 2.0 * np.pi, num=2000)
            ts_neg = np.linspace(-np.pi, 0.0, num=2000)[::-1].copy()
