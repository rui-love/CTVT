...
def compute_loss_2(
    ...
    if M_obs is None:
        inner = (
            weight * torch.sqrt(torch.sum((X_obs - Y_obs) ** 2, dim=1) + eps)
            + (1 - weight) * torch.sqrt(torch.sum((Y_obs_bj - X_obs) ** 2, dim=1) + eps)
        ) ** 2
    else:
        inner = (
            weight * torch.sqrt(torch.sum(M_obs * (X_obs - Y_obs) ** 2, dim=1) + eps)
            + (1 - weight)
            * torch.sqrt(torch.sum(M_obs * (Y_obs_bj - X_obs) ** 2, dim=1) + eps)
        ) ** 2
    outer = torch.sum(inner / n_obs_ot)
    return outer / batch_size
...
nonlinears = {"tanh": torch.nn.Tanh, "relu": torch.nn.ReLU}
    ...
    if nn_desc is None:
        ...
        if len(nn_desc) > 1:
            for i in range(len(nn_desc) - 1):
                ...
                layers.append(
                    torch.nn.Linear(nn_desc[i][0], nn_desc[i + 1][0], bias=bias)
                )
...
class FFNN(torch.nn.Module):
        ...
        if residual:
            ...
            if input_size <= output_size:
                if output_size % input_size == 0:
                    ...
                    raise ValueError(
                        "for residual: output_size needs to be "
            ...
            if input_size > output_size:
                if input_size % output_size == 0:
                    ...
                    raise ValueError(
                        "for residual: input_size needs to be "
    ...
    def forward(self, nn_input, mask=None):
        ...
        if self.case == 0:
            ...
            identity = torch.mean(torch.stack(nn_input.chunk(self.mult, dim=1)), dim=0)
            return identity + out
...
class NJODE(torch.nn.Module):
    ...
    def forward(
        ...
        for i, obs_time in enumerate(times):
            ...
            if self.use_rnn:
                ...
                if self.masked:
                    X_obs_impute = (
                        X_obs * M_obs + (torch.ones_like(M_obs) - M_obs) * Y_bj[i_obs]
                    )
        ...
        if return_path:
            ...
            return h, loss, np.array(path_t), torch.stack(path_h), torch.stack(path_y)
        else:
            return h, loss
    ...
    def get_pred(self, times, time_ptr, X, obs_idx, delta_t, T, start_X, M=None):
        """
        get predicted path
        :param times: see forward
        :param time_ptr: see forward
