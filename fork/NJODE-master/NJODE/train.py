...
def train(
    ...
    **options
    ...
    opt_eval_loss = compute_optimal_eval_loss(dl_val, stockmodel, delta_t, T)
    initial_print += (
    ...
    if model.epoch <= epochs:
        ...
        # send notification
        if SEND:
            SBM.send_notification(text="start training - model id={}".format(model_id))
        initial_print += "\n\nmodel overview:"
        print(initial_print)
        print(model, "\n")
    ...
    while model.epoch <= epochs:
        ...
        for i, b in tqdm.tqdm(enumerate(dl)):
            ...
            unique_idx, counts = np.unique(
                obs_idx.detach().cpu().numpy(), return_counts=True
            )
            n_obs_ot = np.zeros((b_size))
            n_obs_ot[unique_idx] = counts
            n_obs_ot = n_obs_ot.astype(int)
...
def plot_one_path_with_pred(
    ...
    model.eval()
    res = model.get_pred(times, time_ptr, X, obs_idx, delta_t, T, start_X)
    ...
    for i in path_to_plot:
        ...
        for j in range(dim):
            ...
            axs[j].scatter(
                path_t_obs, path_X_obs[:, j], label="observed", color=colors[0]
            )
            axs[j].plot(
