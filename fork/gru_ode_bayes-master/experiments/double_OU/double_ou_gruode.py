...
sys.path.append("C:/Users/Rui_Chen/Desktop/CTVT/fork/gru_ode_bayes-master")
import argparse
...
parser.add_argument(
    "--model_name", type=str, help="Model to use", default="double_OU_gru_ode_bayes"
)
parser.add_argument(
    "--dataset",
    type=str,
    help="Dataset CSV file",
    default="../../gru_ode_bayes/datasets/double_OU/double_OU.csv",
)
parser.add_argument(
    "--jitter",
    type=float,
    help="Time jitter to add (to split joint observations)",
    default=0,
)
parser.add_argument(
    "--seed", type=int, help="Seed for data split generation", default=432
)
parser.add_argument("--full_gru_ode", action="store_true", default=True)
parser.add_argument(
    "--solver", type=str, choices=["euler", "midpoint", "dopri5"], default="euler"
)
...
if args.demo:
    ...
    gru_ode_bayes.paper_plotting.plot_trained_model(
        model_name="double_OU_gru_ode_bayes_demo"
    )
    exit()
...
train_idx, val_idx = train_test_split(
    np.arange(metadata["N"]), test_size=0.2, random_state=args.seed
)
val_options = {"T_val": 4, "max_val_samples": 1}
data_train = data_utils.ODE_Dataset(
    csv_file=args.dataset, idx=train_idx, jitter_time=args.jitter
)
data_val = data_utils.ODE_Dataset(
...
for epoch in range(epoch_max):
    ...
    for i, b in tqdm.tqdm(enumerate(dl)):
        ...
        hT, loss, _, _ = model(
            times, time_ptr, X, M, obs_idx, delta_t=delta_t, T=T, cov=cov
        )
    ...
    with torch.no_grad():
        ...
        for i, b in enumerate(dl_val):
            ...
            hT, loss, _, t_vec, p_vec, h_vec, _, _ = model(
                times,
                time_ptr,
                X,
                M,
            ...
            t_vec = np.around(t_vec, str(delta_t)[::-1].find(".")).astype(
                np.float32
