...
class ODE_DatasetNumpy(Dataset):
    ...
    def __init__(self, times, ids, values, masks):
        ...
        df_masks = pd.DataFrame(
            masks, columns=[f"Mask_{i}" for i in range(masks.shape[1])]
        )
...
class ODE_Dataset(Dataset):
    ...
    def __init__(
        ...
        if panda_df is not None:
            assert (
                csv_file is None
            ), "Only one feeding option should be provided, not both"
            self.df = panda_df
            self.cov_df = cov_df
        ...
        if self.cov_df is None:
            ...
            self.cov_df = pd.DataFrame(
                {"ID": self.df["ID"].unique(), "Cov": num_unique}
            )
        if self.label_df is None:
            num_unique = np.zeros(self.df["ID"].nunique())
            self.label_df = pd.DataFrame(
                {"ID": self.df["ID"].unique(), "label": num_unique}
            )
        ...
        if self.validation:
            ...
            if val_options.get(
                ...
                df_afterIdx = self.df.loc[
                    self.df["Time"] >= val_options["T_val_from"], "ID"
                ].unique()
        ...
        if idx is not None:
            ...
            self.label_df["ID"] = self.label_df["ID"].map(
                map_dict
            )  # Reset the ID index.

        assert self.cov_df.shape[0] == self.df["ID"].nunique()

        self.variable_num = sum(
            [c.startswith("Value") for c in self.df.columns]
        ...
        if self.validation:
            ...
            if val_options.get(
                ...
                self.df_after = (
                    self.df.loc[self.df["Time"] >= val_options["T_val_from"]]
                    .sort_values("Time")
                    .copy()
                )
            else:
            ...
            if val_options.get("T_closest") is not None:
                ...
                df_after_temp["Time_from_target"] = (
                    df_after_temp["Time"] - val_options["T_closest"]
                ).abs()
                df_after_temp.sort_values(
                    by=["Time_from_target", "Value_0"], inplace=True, ascending=True
                )
                df_after_temp.drop_duplicates(subset=["ID"], keep="first", inplace=True)
                self.df_after = df_after_temp.drop(columns=["Time_from_target"])
            else:
                self.df_after = (
                    self.df_after.groupby("ID")
                    .head(val_options["max_val_samples"])
                    .copy()
                )
...
def custom_collate_fn(batch):
    ...
    df_cov = torch.Tensor(np.stack([b["cov"] for b in batch], axis=0))

    # labels = torch.tensor([b["y"] for b in batch])
    labels = torch.tensor(np.stack([b["y"] for b in batch], axis=0))
...
def seq_collate_fn(batch):
    ...
    cov = torch.Tensor(np.stack([b["cov"] for b in batch], axis=0))
    ...
    res["y"] = torch.tensor(np.stack([b["y"] for b in batch], axis=0))
    res["cov"] = cov
