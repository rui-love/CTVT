...
def OU(
    T, dt, r_mu, r_std, theta=10, N_sims=1, sigma=0.1, rho=-0.99, random_theta=False
):
    N_t = int(T // dt)
    mu = (
        np.sqrt(12) * r_std * np.random.uniform(low=-0.5, high=0.5, size=(N_sims, 2))
        + r_mu
    ...
    cov = dt * np.array(
        [[sigma**2, sigma**2 * rho], [sigma**2 * rho, sigma**2]]
    )
...
def OU_sample(
    ...
    for i in range(N):
        variability_num_samples = (
            0.2  # variability in number of samples for each trajectory.
        )
        ...
        if variability_num_samples * 2 * sample_rate * T < 1:
            ...
            num_samples = np.random.randint(
                sample_rate * T * (1 - variability_num_samples),
                sample_rate * T * (1 + variability_num_samples),
            )  # number of sample varies around the mean with 20% variability
        ...
        if full:
            ...
            sample_type = np.random.choice(
                3, num_samples, replace=True, p=[p_single / 2, p_single / 2, p_both]
            )
        ...
        individual_data = pd.DataFrame(
            np.concatenate(
                (
                    i * np.ones((num_samples, 1)),
                    dt * np.expand_dims(sample_times, 1),
                    samples,
                    mask,
                    covs,
                ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rho", type=float, help="Correlation between the two variables.", default=0.99
    )
    parser.add_argument(
        "--prefix", type=str, help="Prefix for generated data", default="double_OU"
    )
    parser.add_argument(
        "--random_r",
        action="store_true",
        help="Generates random targets (r) from uniform distribution with mean 1 (-1) and std = 0.5",
        default=True,
    )
    parser.add_argument(
        "--max_lag",
        type=float,
        help="Shift trajectories with a random positive lag. Insert max lag allowed.",
        default=0,
    )
    parser.add_argument(
        "--random_theta",
        action="store_true",
        help="Generates random thetas for each sample",
    )
